...
<body>
    ...
    <script>
        ...
        function getQuadrantInfo(x, y) {
            if (x >= 0 && y >= 0) {
                return { name: "領先 (Leading)", color: "var(--color-lead)", bgClass: "lead" };
            } else if (x >= 0 && y < 0) {
                return { name: "轉弱 (Weakening)", color: "var(--color-rotate)", bgClass: "rotate" };
            } else if (x < 0 && y < 0) {
                return { name: "落後 (Lagging)", color: "var(--color-outflow)", bgClass: "outflow" };
            } else {
                return { name: "改善 (Improving)", color: "var(--color-watch)", bgClass: "watch" };
            }
        }
        ...
        // RRG Time-Lapse Loop — pure coordinate animation, zero physics
        function startTimeLapse(data) {
            ...
            const flowKey = tf === "5d" ? "flow_5d_m" : "flow_20d_m";

            // Pre-build a lookup map: sector_name → array of {px, py} screen coords
            // Index 0 = oldest (Day -5), last index = today
            const coordMap = {};
            data.forEach(d => {
                const history = d.history && d.history[tf] ? d.history[tf] : [];
                const pts = history.map(pt => ({
                    px: xScale(pt.x),
                    py: yScale(pt.y)
                }));
                // Append today's real coordinate as the final point
                pts.push({
                    px: xScale(d[flowKey]),
                    py: yScale(d.acceleration_m)
                });
                coordMap[d.sector_name] = pts;
            });

            // Determine the maximum number of steps (should be 6: indices 0..5)
            const maxSteps = Math.max(...Object.values(coordMap).map(arr => arr.length));

            currentDayIndex = 0;

            function moveBubblesTo(dayIdx, duration, easing) {
                d3.selectAll(".bubble-g")
                    .transition("timelapse")
                    .duration(duration)
                    .ease(easing)
                    .attr("transform", function(d) {
                        const pts = coordMap[d.sector_name];
                        if (pts && dayIdx < pts.length) {
                            const coord = pts[dayIdx];
                            d.current_anim_x = coord.px;
                            d.current_anim_y = coord.py;
                            return `translate(${coord.px}, ${coord.py})`;
                        }
            ...
            function playStep() {
                ...
                // Move all bubbles to this day's coordinates
                moveBubblesTo(dayIdx, 800, d3.easeLinear);

                if (currentDayIndex < maxSteps - 1) {
                    // Not yet at today — advance to next day
                    currentDayIndex++;
                    timeLapseTimer = setTimeout(playStep, 1000);
                } else {
                    // Reached today — pause 3 seconds, then reset & loop
                    timeLapseTimer = setTimeout(() => {
                        // Instant snap back to Day -5 (index 0)
                        currentDayIndex = 0;
                        moveBubblesTo(0, 300, d3.easeCubicOut);

                        // Wait for snap-back to finish, then restart
                        timeLapseTimer = setTimeout(playStep, 500);
                    }, 3000);
                }
            }

            // Kick off: place bubbles at Day -5 instantly, then start stepping
            moveBubblesTo(0, 0, d3.easeLinear);
            timeLapseTimer = setTimeout(playStep, 400);
        }
        ...
        async function loadData() {
            try {
                ...
                // 板塊名稱映射機制 (Name Mapping)
                const nameMapping = {
                    "轉單受惠與國防儲能": "軍工概念股"
                };
                const mapName = name => nameMapping[name] || name;

                if (cachedData) {
                    cachedData.forEach(sector => {
                        sector.sector_name = mapName(sector.sector_name);
                        if (sector.individual_stocks) {
                            sector.individual_stocks.forEach(st => {
                                if (st.sector) st.sector = mapName(st.sector);
                            });
                        }
                        if (sector.bottom_fishing_stocks) {
                ...
                if (cachedWhaleStocks) {
                    cachedWhaleStocks.forEach(st => {
                        if (st.sector) st.sector = mapName(st.sector);
                    });
                }
        ...
        function updateUI() {
            ...
            displayData.forEach(sector => {
                const customSectors = ["台積先進封裝設備", "軍工概念股", "高值化半導體材料"];
        ...
        function renderWhaleLockedStocks(whaleStocks, min, max) {
            ...
            filtered.forEach(stock => {
                ...
                const isCustom = ["台積先進封裝設備", "軍工概念股", "高值化半導體材料"].includes(stock.sector);
        ...
        function showSectorDetail(sectorName) {
            ...
            // Fill text
            const isCustom = ["台積先進封裝設備", "軍工概念股", "高值化半導體材料"].includes(sectorName);
        ...
        function renderQuadrantChart(data) {
            ...
            // 2.5 Grid concentric circles (RRG style)
            ...
            // Helper to get coordinates at dayIndex (0 to 5) for sector d
            function getSectorPointAt(d, dayIndex) {
                const tf = currentTimeFrame;
                const history = d.history && d.history[tf] ? d.history[tf] : [];
                if (dayIndex < history.length) {
                    return { x: history[dayIndex].x, y: history[dayIndex].y };
                }
                return { x: d[flowKey], y: d.acceleration_m };
            }

            // 3. Render Tails Group (static background track trails under the bubbles)
            const tailsGroup = svg.append("g").attr("class", "tails");
            
            const tailPaths = tailsGroup.selectAll(".tail-path")
                .data(data, d => d.sector_name)
                .enter()
                .append("path")
                .attr("class", "tail-path")
                .attr("id", d => `tail-path-${d.sector_name.replace('/', '-')}`)
                .attr("d", d => {
                    const tf = currentTimeFrame;
                    const history = d.history && d.history[tf] ? d.history[tf] : [];
                    const points = history.map(pt => [xScale(pt.x), yScale(pt.y)]);
                    points.push([xScale(d[flowKey]), yScale(d.acceleration_m)]); // 今日 (Day 0)
                    return d3.line()(points);
                })
                .style("stroke", d => {
                    const customSectors = ["台積先進封裝設備", "軍工概念股", "高值化半導體材料"];
                    return customSectors.includes(d.sector_name) ? "#ff007f" : getQuadrantInfo(d[flowKey], d.acceleration_m).color;
                })
                .style("fill", "none")
                .style("--sector-color", d => {
                    const customSectors = ["台積先進封裝設備", "軍工概念股", "高值化半導體材料"];
                    return customSectors.includes(d.sector_name) ? "#ff007f" : getQuadrantInfo(d[flowKey], d.acceleration_m).color;
                });

            // 4. Render Bubbles Group (initially placed at Day -5)
            const bubbleGroup = svg.selectAll(".bubble-g")
                .data(data, d => d.sector_name)
                .enter()
                .append("g")
                .attr("class", "bubble-g")
                .attr("id", d => `bubble-g-${d.sector_name.replace('/', '-')}`)
                .attr("transform", d => {
                    const pt = getSectorPointAt(d, 0); // Day -5
                    d.current_anim_x = xScale(pt.x);
                    d.current_anim_y = yScale(pt.y);
                    return `translate(${xScale(pt.x)}, ${yScale(pt.y)})`;
                });
            ...
            bubbleGroup.append("circle")
                ...
                .on("mouseover", function(event, d) {
                    ...
                    hoverLinkageActive(d.sector_name, event);
        ...
        function transitionQuadrantChart(data) {
            ...
            // Transition tail paths to new history coordinate shapes
            svg.selectAll(".tail-path")
                .data(data, d => d.sector_name)
                ...
                .attr("d", d => {
                    ...
                    const history = d.history && d.history[tf] ? d.history[tf] : [];
                    const points = history.map(pt => [xScale(pt.x), yScale(pt.y)]);
                    points.push([xScale(d[flowKey]), yScale(d.acceleration_m)]);
                    return d3.line()(points);
                })
                .style("stroke", d => {
                    const customSectors = ["台積先進封裝設備", "軍工概念股", "高值化半導體材料"];
            ...
            // Restart time-lapse animation loop with new scale domain mapping
            startTimeLapse(data);
        }

        // Hover Linkages: Fade out irrelevant elements & show projection + tooltip
        function hoverLinkageActive(sectorName, event = null) {
            d3.select(".tails").classed("has-hover", true);
            d3.selectAll(".tail-path").classed("hovered", d => d.sector_name === sectorName);
            ...
            const targetBubbleG = d3.select(`#bubble-g-${escapedName}`);
            
            if (!targetBubbleG.empty()) {
                targetBubbleG.raise();
                const d = targetBubbleG.datum();
                const flowKey = currentTimeFrame === "5d" ? "flow_5d_m" : "flow_20d_m";
                
                // Get current animation coordinates
                const tf = currentTimeFrame;
                const history = d.history && d.history[tf] ? d.history[tf] : [];
                const curX = d.current_anim_x !== undefined ? d.current_anim_x : (history.length > 0 ? xScale(history[0].x) : xScale(d[flowKey]));
                const curY = d.current_anim_y !== undefined ? d.current_anim_y : (history.length > 0 ? yScale(history[0].y) : yScale(d.acceleration_m));

                targetBubbleG.select(".bubble")
                    .transition().duration(150)
                    .attr("r", sizeScale(d.size_20d_m) * 1.2)
                ...
                // Calculate Tooltip position based on event source
                let tooltipX, tooltipY;
                const isListHover = event && !(event.target instanceof SVGElement || event.currentTarget instanceof SVGElement);
                
                if (isListHover) {
                    tooltipX = event.pageX + 15;
                    tooltipY = event.pageY + 15;
                } else {
                    const bubbleElement = document.getElementById(`bubble-g-${escapedName}`);
                    if (bubbleElement) {
                        const rect = bubbleElement.getBoundingClientRect();
                        const tooltipWidth = tooltip.node().offsetWidth || 280;
                        const tooltipHeight = tooltip.node().offsetHeight || 180;
                        
                        tooltipX = rect.left + window.scrollX + rect.width / 2 - tooltipWidth / 2;
                        tooltipY = rect.top + window.scrollY - tooltipHeight - 10;
                    } else {
                        tooltipX = event ? event.pageX + 15 : curX - 10;
                        tooltipY = event ? event.pageY + 15 : curY - 180;
                    }
                }
        ...
        function hoverLinkageReset() {
            // Reset tails interactive styling
            d3.select(".tails").classed("has-hover", false);
            d3.selectAll(".tail-path").classed("hovered", false);
            ...
            d3.selectAll(".bubble-g").each(function(d) {
                ...
                if (!bubble.empty()) {
                    bubble.transition().duration(150)
                        .attr("r", sizeScale(d.size_20d_m))
                        .style("fill-opacity", d.sector_name === selectedSectorName ? 0.95 : (customSectors.includes(d.sector_name) ? 0.85 : 0.6))
                        .attr("stroke", d.sector_name === selectedSectorName ? "#ffffff" : (customSectors.includes(d.sector_name) ? "#ff007f" : getQuadrantInfo(d[flowKey], d.acceleration_m).color))
                        .style("filter", d.sector_name === selectedSectorName ? "drop-shadow(0 0 12px #ffffff)" : "none");
                }
            });

            d3.select("#proj-x").transition().duration(150).style("opacity", 0);
            d3.select("#proj-y").transition().duration(150).style("opacity", 0);
            d3.select("#map-tooltip").transition().duration(150).style("opacity", 0);
